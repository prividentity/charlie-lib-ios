...
/**
* \brief Perform Faces Detect Operation 
* 
* @param *session_ptr:              The session buffer which is already initialized using privid_initialized_session 
* @param *user_conf:                User configuration in the JSON parameters. Any configuration parameter provided
*                                   in the configuration JSON shall overwrite the default value of the parameter. 
...
*                                   width
* @param image_height:              Height of the one input image.
* @param[out] **faces_out                Returns cropped faces image if not null. The caller will have to de-allocate memory after using it.
* @param[out] *faces_out_len             Length of the cropped faces image buffer;
* @param[out] **operation_result_out:    Output buffer which contains the result of the operation. The API will
*                                   allocate desired memory to this buffer, it is the responsibility of 
*                                   caller to free the memory after the call using privid_free_char_buffer() API
*                                   NULL is a valid input if caller does not require the output
...
*
*/
PRIVID_API_ATTRIB int privid_detect_faces(
        void *session_ptr, const char *user_conf, const int conf_len,
        const uint8_t *input_image, const int image_width, const int image_height,
        uint8_t **faces_out, int *faces_out_len,
        char **operation_result_out, int *operation_result_out_len);

/**
* \brief Compute Embedding of input image(s). API can take upto 20 input images of the same person. All images are cropped output of the privid_detect_faces() API. 
*
* @param *session_ptr:              The session buffer which is already initialized using privid_initialized_session
* @param *user_conf:                User configuration in the JSON parameters. Any configuration parameter provided
*                                   in the configuration JSON shall overwrite the default value of the parameter.
...
*                                   This can also be null, in such case, default configuration shall be used
* @param conf_len:                  Length of the configuration buffer.
* @param *input_images:             Input image(s) buffer. all sizes are required to be equal to 224x224 
* @param image_count:               Image Count
* @param image_size:                Image size in bytes.
* @param image_width:               Width of the cropped input image.
* @param image_height:              Height of the cropped input image.
* @param[out] **embeddings_out:
* @param[out] *embeddings_out_len:
* @param remove_bad_embeddings:
* @param[out] **augmentations_out:
* @param[out] *augmentations_out_len:
* @param[out] **operation_result_out:    Output buffer which contains the result of the operation. The API will
*                                   allocate desired memory to this buffer, it is the responsibility of
*                                   caller to free the memory after the call using privid_free_char_buffer() API
*                                   NULL is a valid input if caller does not require the output
...
*
*/
PRIVID_API_ATTRIB int privid_compute_embeddings(
        void* session_ptr, const char* user_conf, const int conf_len,
        const uint8_t* input_images,const int image_count,
        const int image_size, const int image_width, const int image_height,
